...
func (note *Note) readBody(maxLines int) (string, int, error) {
	path := note.FilePath()
	f, err := os.Open(path)
	if err != nil {
	...
	for {
		if maxLines > 0 && readLines >= maxLines {
			break
		}
		b, err := r.ReadBytes('\n')
		if err != nil {
