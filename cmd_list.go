...
func printOnelineNotesTo(out io.Writer, notes []*Note) error {
	...
	for i, note := range notes {
		...
		green.Fprint(bufOut, filepath.FromSlash(note.Category))
		bufOut.WriteRune(filepath.Separator)
		yellow.Fprint(bufOut, note.File)
		bufOut.WriteString(pad)

		pad = strings.Repeat(" ", max[1]-tw[i][1]+1) // +1 for separator
		bold.Fprint(bufOut, strings.Join(note.Tags, ","))
		bufOut.WriteString(pad)

		bufOut.WriteString(note.Title)
		bufOut.WriteRune('\n')
	}
